...
const dtod = "6281391420170@s.whatsapp.net"
...
{ remoteJid: "6281391420170-1613049930@g.us" } : {})
},
message: { 
"videoMessage": { 
"title": `${tampilUcapan}`,
...
try {
		...
		const katalog = (teks) => {
             res = helga.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© KOV BOTZZ_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             helga.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = helga.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6282138919347-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IlpQRKWDZed0GgVHmYJUbl', "groupName": `${NamaBot}`, "footerText": "*_© KOV BOTZZ_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
const daftar2 = '```Ketik Tombol Di Bawah Untuk Mendaftar Ke Database KOV BOTZZ```'
...
⚠️ Segala kebijakan dan ketentuan KOV BOTZZ di pegang oleh owner dan segala perubahan kebijakan, sewaktu waktu owner berhak mencabut, memblokir user(*﹏*) 
...
YouTube KOVBOTZZ : https://youtube.com/channel/UC52l0AfvQy2eliMOMVpRWtA
`
 ...
 rows: [
                          {
                           ...
                           {
                              "title": "Owner Bot🐣",
                              "description": 'Owner Bot KOV BOTZZ!',
                              "rowId": ""
...
case 'bugreport':
              ...
              helga.sendMessage('6281391420170@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
              case 'verify':
...
case 'cecan':
            ...
            reply (mess.wait)
              ...
              imageMsg = (await helga.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'𝚉𝚞𝚔𝚊𝙲𝚑𝚊𝚗 𝚅3.0.0', imageMessage: imageMsg,
              contentText:`Nih Kak!\nJangan Lupa Subscribe YT KOV BOTZZs`,buttons,headerType:4}
              prep = await helga.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              helga.relayWAMessage(prep)
              break                                           
...
case 'ss':
             ...
             buttons = [{buttonId: `${prefix}menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await helga.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'𝚉𝚞𝚔𝚊𝙲𝚑𝚊𝚗 𝚅3.0.0', imageMessage: imageMsg,
              contentText:`Nih Kak!\nJangan Lupa Subscribe YT KOV BOTZZ!`,buttons,headerType:4}
              prep = await helga.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              helga.relayWAMessage(prep)
              break
                    case 'tiktokdl': 
...
┃💌 : YouTube : _*KOV BOTZZ's*_
┗━━⬣`
				sendButLocation(from, `${memek}`, `${tod}Subscribe Channel\nKOV BOTZZ!!${tod}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'rdp':
case 'vps':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
...
Minat?Chat
https://wa.me/6281391420170`
				sendButLocation(from, `${memek}`, `Subscribe Channel\nKOV BOTZZ!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'jasrun':
case 'jasarun':
case 'kebutuhanbot':
  ...
  //── 「 Pembatas 」 ──//               
			...
			case 'runtime':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				...
				sendButLocation(from, `${memek}`, `${tod}Subscribe Channel\nKOV BOTZZ!!${tod}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'AUTHOR🍁'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
			case 'setbio':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply('Teksnya?')
